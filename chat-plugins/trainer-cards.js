 ...
 */
 ...
 /*--
    Trainer Cards
        Este plugin permite crear tarjetas de
        entrenador personalizadas a base de
        HTML y CSS como una roomintro.
...
const TC_PATH = P_DATA_DIR + 'trainer-cards.json';

let trainerCards = {};

function loadTrainerCards() {
	try {
		trainerCards = serialize.unserialize(fs.readFileSync(TC_PATH, 'utf8'));
...
function saveTrainerCards() {
	fs.writeFileSync(TC_PATH, serialize.serialize(trainerCards));
	Object.assign(Chat.commands, trainerCards);
}
